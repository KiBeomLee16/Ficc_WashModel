...
public record AlertBusinessKey(
        LocalDate tradeDate,
        String assetClass,
        String instrumentId,
    ...
    public static AlertBusinessKey from(Alert alert) {
        ...
        List<Trade> trades = alert.relatedTrades()
                .stream()
        ...
        return new AlertBusinessKey(
                lastTrade.timestamp().toLocalDate(),
                firstTrade.assetClass(),
                firstTrade.instrumentId(),
                firstTrade.maturity(),
                firstTrade.currency(),
                distinctJoined(trades, Trade::traderId),
                distinctJoined(trades, Trade::counterpartyId)
        );
    }

    public String hash(String matchType) {
        String input = normalize(matchType)
                + "|" + tradeDate
                + "|" + normalize(assetClass)
                + "|" + normalize(instrumentId)
                + "|" + maturityDate
                + "|" + normalize(currency)
                + "|" + normalize(traderId)
