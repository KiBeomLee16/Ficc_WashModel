...
public abstract class AbstractSurveillanceModel {
    ...
    protected AbstractSurveillanceModel(
     ...
     */
    public boolean dispatchAlert(long requestId, ModelConfig modelConfig, LocalDate businessDate, Alert alert, String alertPayload) {
    ...
    protected BigDecimal getAlertThreshold(ModelConfig modelConfig, String thresholdName, LocalDate businessDate) {
        ...
        try (Connection connection = getConnection();
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                if (resultSet.next()) {
                    ...
                    if (threshold.signum() < 0) {
                        throw new IllegalStateException("Threshold cannot be negative for appid="
                                + modelConfig.appId() + ", modelid=" + modelConfig.modelId()
                                + ", region=" + modelConfig.region()
                                + ", thresholdName=" + normalizedThresholdName
            ...
            throw new IllegalStateException("Failed to load threshold for appid="
                    + modelConfig.appId() + ", modelid=" + modelConfig.modelId()
                    + ", region=" + modelConfig.region()
                    + ", thresholdName=" + normalizedThresholdName
        ...
        throw new IllegalArgumentException("No active threshold found for appid="
                + modelConfig.appId() + ", modelid=" + modelConfig.modelId()
                + ", region=" + modelConfig.region()
                + ", thresholdName=" + normalizedThresholdName
                + ", businessDate=" + businessDate);
    ...
    protected int getThresholdLookupDays(ModelConfig modelConfig, String thresholdName, LocalDate businessDate) {
        ...
        try (Connection connection = getConnection();
            ...
            throw new IllegalStateException("Failed to load threshold lookup days for appid="
                    + modelConfig.appId() + ", modelid=" + modelConfig.modelId()
                    + ", region=" + modelConfig.region()
                    + ", thresholdName=" + normalizedThresholdName
                    + ", businessDate=" + businessDate, exception);
    ...
    protected Trade toTrade(ResultSet resultSet) throws SQLException {
        return new Trade(
                ...
                resultSet.getTimestamp("trade_timestamp").toLocalDateTime(),
                resultSet.getString("asset_class"),
                resultSet.getString("instrument_id"),
                resultSet.getDate("maturity").toLocalDate(),
                resultSet.getString("currency"),
                Side.valueOf(resultSet.getString("side").toUpperCase()),
                resultSet.getBigDecimal("quantity"),
                resultSet.getBigDecimal("price"),
                resultSet.getString("counterparty_id"),
                resultSet.getString("account_id"),
                resultSet.getString("beneficial_owner"),
                resultSet.getString("trader_id"),
                resultSet.getString("desk"),
    ...
    private String tradeToJson(Trade trade, int parentSpaces) {
        ...
        appendStringField(json, spaces, "timestamp", trade.timestamp() == null ? null : trade.timestamp().toString(), true);
        ...
        appendStringField(json, spaces, "maturity", trade.maturity() == null ? null : trade.maturity().toString(), true);
    ...
    private void appendStringField(StringBuilder json, int spaces, String fieldName, String value, boolean includeComma) {
        indent(json, spaces).append(quote(fieldName)).append(": ").append(quote(value));
        appendCommaAndNewLine(json, includeComma);
    }

    private void appendBigDecimalField(StringBuilder json, int spaces, String fieldName, BigDecimal value, boolean includeComma) {
