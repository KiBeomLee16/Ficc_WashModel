...
public class TradeCsvReader {
    ...
    private Trade parseTrade(String line) {
        ...
        if (columns.size() != EXPECTED_COLUMNS) {
            throw new IllegalArgumentException("Expected " + EXPECTED_COLUMNS + " columns but found "
                    + columns.size() + ": " + line);
        }

        return new Trade(
                columns.get(0),
                LocalDateTime.parse(columns.get(1)),
                columns.get(2),
                columns.get(3),
                LocalDate.parse(columns.get(4)),
                columns.get(5),
                Side.valueOf(columns.get(6).trim().toUpperCase()),
                new BigDecimal(columns.get(7)),
                new BigDecimal(columns.get(8)),
                columns.get(9),
                columns.get(10),
                columns.get(11),
                columns.get(12),
