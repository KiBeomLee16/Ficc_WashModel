...
public class CalibrationResultRepository {
	...
	public boolean saveIfNew(long requestId, ModelConfig modelConfig, LocalDate businessDate, Alert alert,
		...
		try (Connection connection = getConnection()) {
			...
			try {
				...
				long calibrationAlertHistoryId = insertCalibrationAlertHistory(connection, requestId, modelConfig,
						businessDate, alert, alertPayload, relatedTradeIds, alertFingerprint, businessKey,
						firstTradeDate, lastTradeDate, thresholdSnapshot);
			...
			throw new IllegalStateException("Failed to save calibration alert history for alertId=" + alert.alertId(),
					exception);
		}
	}
	...
	public List<CalibrationAlertHistoryResult> findByRequestId(long requestId) {
		...
		try (Connection connection = getConnection();
			...
			throw new IllegalStateException("Failed to search calibration alert history for requestId=" + requestId,
					exception);
		}
	}
		...
		try (Connection connection = getConnection();
			...
			try (ResultSet resultSet = statement.executeQuery()) {
				if (resultSet.next()) {
					...
					LOGGER.info(
							"Deleted existing calibration alert history before refresh: requestId={}, alerts={}, drillOutTrades={}.",
			...
			throw new IllegalStateException("Failed to delete calibration alert history for requestId=" + requestId,
					exception);
		}
	}
	...
	private ThresholdSnapshot loadThresholdSnapshot(Connection connection, ModelConfig modelConfig)
			throws SQLException {
		try (CallableStatement statement = connection.prepareCall(THRESHOLD_SNAPSHOT_CALL)) {
			...
			try (ResultSet resultSet = statement.executeQuery()) {
				if (resultSet.next()) {
					return new ThresholdSnapshot(resultSet.getBigDecimal("one_time_min_total_amount"),
	...
	private long insertCalibrationAlertHistory(Connection connection, long requestId, ModelConfig modelConfig,
			...
			ThresholdSnapshot thresholdSnapshot) throws SQLException, DuplicateCalibrationAlertHistoryException {
	...
	private void insertCalibrationAlertDrillOutRows(Connection connection, long calibrationAlertHistoryId, Alert alert)
			throws SQLException {
	...
	private CalibrationAlertHistoryResult toCalibrationAlertHistoryResult(ResultSet resultSet) throws SQLException {
		...
		return new CalibrationAlertHistoryResult(resultSet.getLong("calibration_alert_history_id"),
				resultSet.getString("alert_id"), resultSet.getLong("request_id"), resultSet.getInt("appid"),
				resultSet.getInt("modelid"), resultSet.getString("region"), resultSet.getString("alert_type"),
				resultSet.getString("match_type"), resultSet.getDate("business_date").toLocalDate(),
				resultSet.getDate("first_trade_date").toLocalDate(), resultSet.getDate("last_trade_date").toLocalDate(),
				resultSet.getString("related_trade_ids"), resultSet.getString("alert_business_key_hash"),
				resultSet.getDate("trade_date").toLocalDate(), resultSet.getString("asset_class"),
				resultSet.getString("instrument_id"), resultSet.getDate("maturity_date").toLocalDate(),
				resultSet.getString("currency"), resultSet.getString("trader_id"),
				resultSet.getString("counterparty_id"), resultSet.getString("alert_payload"),
				resultSet.getBigDecimal("one_time_min_total_amount"),
				resultSet.getBigDecimal("cumulative_min_total_amount"),
				resultSet.getBigDecimal("quantity_tolerance_percent"),
				resultSet.getBigDecimal("total_amount_tolerance_percent"), resultSet.getInt("cumulative_lookup_days"),
				resultSet.getString("dispatch_status"), createdAt == null ? null : createdAt.toLocalDateTime());
	}
